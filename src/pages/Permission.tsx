...
export function Permission() {
  ...
  const [list_Permission, setItems] = useState<Permissions[]>([]);
  let currentPage = 1;


  const getPermission = async (currentPage: number, limit: number) => {
    try {
      ...
      console.error("Erreur lors du chargement des Permissions Vérifié :", error);
    }
  };

  const fetchPermission = async (currentPage: number, limit: number) => {
    const res = await fetch(`${backendUrl}/api/vehiclecheck/${1}/${currentPage}/${limit}`,
    { mode: 'cors' });
    const data = await res.json();
    return data;
  ...
  const handlePageClick = async (data: any) => {
    ...
    const commentsFormServer = await fetchPermission(currentPage, limit);
    setItems(commentsFormServer);
    window.scrollTo(0, 0);
  };

  useEffect(() => {
    getPermission(currentPage, limit);
  }, [currentPage, limit]);
    ...
    <>
      <div className="row">
        <div className="col-md-6 col-sm-12">
          <h4>
            <i className="las la-car" data-rel="bootstrap-tooltip" title="Increased"></i>{translate('Verified Permissions')} ({total})
          </h4>
        </div>
