...
export interface BonPDFData {
  ...
  statut?: string;
  pieces: BonPDFPiece[]; // <-- obligatoire pour éviter l'erreur
}
...
export function generateBonPDF(d: BonPDFData) {
  ...
  doc.text(
    `Date : ${
      d.date_creation ? toTimestamp(d.date_creation) : toTimestamp(new Date().toISOString())
    }`,
    rightX,
    startY + lineHeight * 2
  );
